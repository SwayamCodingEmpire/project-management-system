...
public interface TimeSheetRepository extends JpaRepository<TimeSheet, Integer> {
	...
	@Query("SELECT new com.cozentus.pms.dto.TimesheetFlatDTO(" +
		       ...
		       "LEFT JOIN r.timeSheets ts ON ts.date BETWEEN :startDate AND :endDate AND ts.enabled = true " +
		       "WHERE r.resource.empId = :empId " +
		       "AND r.enabled = true " +
		       "AND r.allocationCompleted = false " +
		       "AND (:projectCode IS NULL OR :projectCode = '' OR p.projectCode = :projectCode)")
		List<TimesheetFlatDTO> findAllTimesheetByEmpIdAndDateBetween(
		        String empId, LocalDate startDate, LocalDate endDate, String projectCode);
	...
	@Query("SELECT new com.cozentus.pms.dto.TimesheetSummaryDTO("
			...
			+ "AND ts.approvalStatus <> com.cozentus.pms.helpers.ApprovalStatus.PENDING "
	...
	@Query("SELECT new com.cozentus.pms.dto.TimesheetForManagerFlatDTO("
	        ...
	        + "LEFT JOIN ra.project p "
	        + "LEFT JOIN ra.resource r "
	        + "WHERE r.empId = :resourceId "
	        + "AND (p.projectManager.empId = :managerId OR p.deliveryManager.empId = :managerId) "
	        + "AND ts.date BETWEEN :startDate AND :endDate "
	        + "AND ts.enabled = true "
	        + "AND ts.approvalStatus IN ('PENDING','SUBMITTED') "
	        + "AND (:projectCode IS NULL OR :projectCode = '' OR LOWER(p.projectCode) = LOWER(:projectCode))")
